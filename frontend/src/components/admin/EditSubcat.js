...
function EditSubcat(props) 
{
    ...
    useEffect(() => {
        axios.get(`/api/singlesubcat/${cid}`).then(res=>{
    ...
    useEffect(() => {
        document.title= "Portfolio | rentizee | Edit Subcat";
    }, [])
    if(loading){
    ...
    const updateCat = (e)=>{
        ...
        axios.post(`/api/updsubcat/${cid}`, data).then(res=>{
            if(res.data.status===200){
 ...
 <div className="container-fluid px-4">
        <h1 className="mt-4 text-success text-uppercase " >Edit Sub Category
    ...
    <div className="tab-content" id="myTabContent">
        <div className="tab-pane fade show active card-body border" id="home" role="tabpanel" aria-labelledby="home-tab">
            ...
            <div className="form-group mb-3">
                ...
                <input readnoly disabled type="text" name="slug" value={categoryInput.slug} className="form-control" />
            ...
            <div className="form-group mb-3">
                ...
                <select className="form-control form-select custom-select" onChange={(e)=>setParentcat(e.target.value)} >
                    ...
                    {
                        cat.map((item, key)=>{
                        return (<option className="text-uppercase" key={item.id} value={item.value} selected={categoryInput.parentcat} >{item.name}</option>);
                        })
                    }
                </select>
                <small className="text-danger text-lowercase">{inperror.parentcat}</small>
            </div>
...
export default withRouter(EditSubcat);
