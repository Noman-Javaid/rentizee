...
function EditUser(props) {
    ...
    useEffect(() => {
        document.title= "Portfolio | rentizee | Edit User";
    }, [])
    if(loading){
    ...
    const registerSubmit = (e) =>{ 
        ...
        axios.post(`/api/upduser/${uid}`, formdata).then(res =>{
            if (res.data.status === 200) {
    ...
    return (
        ...
        <h1 className="mt-4 text-success text-uppercase " >Update User</h1>
        <nav>
            ...
            <small className="text-danger">{error.role ? error.role : ''}</small><br/>
            <small className="text-danger">This User Role is {registerInput.role} User</small>
            ...
            <button type="submit" className="btn btn-info text-light float-end mt-4">Update User</button>
                </form>
